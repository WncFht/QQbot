...
class Database:
    ...
    def backup(self, max_backups=5):
        ...
        try:
            ...
            backup_path = f"{os.path.splitext(self.database_path)[0]}_{backup_time}.db"
            ...
            # 清理旧备份（保留最近几个）
            backup_files = [f for f in os.listdir('.') if f.startswith(os.path.splitext(self.database_path)[0]) and f.endswith('.db') and f != self.database_path]
